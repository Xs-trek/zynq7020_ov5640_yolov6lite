...
static int tracker_verbose_log = 0;
static float tracker_diag_interval_sec = 0.0f;
...
static void tracker_load_log_config(void)
{
    const char *diag = getenv("CAM_TRACKER_DIAG_EVERY");

    tracker_verbose_log = env_flag_enabled("CAM_TRACKER_VERBOSE");
    tracker_diag_interval_sec = 0.0f;
    if (diag && diag[0] != '\0') {
        float v = strtof(diag, NULL);
        if (v > 0.0f)
            tracker_diag_interval_sec = v;
    }
}
...
void tracker_write_isp_offset(int x, int y)
{
    ...
    if (ov5640_apply_isp_offset(x, y) < 0) return;
...
static void scaler_restore(void) {
    ...
    ov5640_set_scaler_enabled(1);
    scaler_disabled = 0;
}

static void scaler_disable(void) {
    if (scaler_disabled) return;
    ov5640_set_scaler_enabled(0);
    scaler_disabled = 1;
}
...
void tracker_apply_cmd(tracker_t *trk, track_cmd_t cmd, int tid)
{
    ...
    case TRACK_CMD_START: {
        ...
        app_state_store_int(&g_state.tracking_active, 1);
        app_state_store_int(&g_state.tracking_target_id, trk->target_id);
                               ...
                               float time_sec)
{
    ...
    if (best >= 0) {
        ...
        sm_push(trk, 0);
        if (tracker_verbose_log && det_count > 0) {
...
void tracker_predict(tracker_t *trk, float time_sec)
{
    ...
    float pred_cy = 0.0f;
    {
        ...
        pthread_mutex_lock(&g_state.pred_mutex);
        g_state.pred_box.cx = pred_cx;
        g_state.pred_box.cy = pred_cy;
    ...
    /* Optional diagnostic log; disabled by default on the frame path. */
    static float last_diag = 0;
    if (tracker_diag_interval_sec > 0.0f &&
        time_sec - last_diag >= tracker_diag_interval_sec) {
        last_diag = time_sec;
